...
export const latestProductVersions = (products: Product[]) => Object.values(products.reduce<Record<string, Product>>((latest, item) => (!latest[item.product_id] || item.version > latest[item.product_id].version ? { ...latest, [item.product_id]: item } : latest), {})).sort((a, b) => (b.created_at ?? '').localeCompare(a.created_at ?? '')).slice(0, 6)
...
export default function App() {
  ...
  const refreshRecentProducts = async () => { try { setRecentProducts(await listProducts()) } catch { /* History is optional; keep the current list visible. */ } }
