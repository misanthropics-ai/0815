...
def _product_from_row(r: dict) -> dict:
    return {
        ...
        "raw_text": r["raw_text"],
        "attributes": _uj(r["attributes_json"], []),
...
def latest_version(product_id: str) -> Optional[int]:
    ...
    try:
        row = conn.execute(
            "SELECT MAX(version) AS v FROM products WHERE product_id=?", (product_id,)
        ).fetchone()
...
def get_product(product_id: str, version: Optional[int] = None) -> Optional[dict]:
    ...
    try:
        row = conn.execute(
            "SELECT * FROM products WHERE product_id=? AND version=?", (product_id, version)
        ).fetchone()
...
_RUN_JSON_FIELDS = {
    ...
    "funnel_summary": "funnel_summary_json",
...
def get_run(run_id: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            "run_id": r["run_id"],
            "config": _uj(r["config_json"], {}),
            "status": r["status"],
            "stage": r["stage"],
            "progress": _uj(r["progress_json"], {}),
            "funnel_summary": _uj(r["funnel_summary_json"]),
            "evidence": _uj(r["evidence_json"]),
            "report": _uj(r["report_json"]),
            "error": r["error"],
            "created_at": r["created_at"],
...
def list_runs(limit: int = 50) -> list[dict]:
    ...
    try:
        cur = conn.execute(
            "SELECT run_id, config_json, status, stage, progress_json, error, created_at, updated_at "
    ...
    try:
        conn.executemany(
            ...
            [
                (
                    i["intent_id"],
                    i.get("run_id", "library"),
                    i["text"],
                    i.get("cluster_id", "other"),
                    i.get("cluster_label"),
                    _j(i.get("attributes", [])),
                    i.get("persona"),
                    i.get("persona_id"),
                    _j(i.get("persona_profile")) if i.get("persona_profile") else None,
                    i.get("language", "en"),
                    i.get("source", "generated"),
                    i.get("created_at") or now_iso(),
                )
                for i in intents
            ],
        )
    ...
    try:
        conn.execute(
            ...
            (
                ...
                r.get("text", ""),
                _j(r.get("citations", [])),
                _j(r.get("search_queries", [])),
                _j(r.get("ground_truth")) if r.get("ground_truth") else None,
                r.get("latency_ms"),
                r.get("error"),
                r.get("cache_key"),
...
def _response_from_row(r: dict) -> dict:
    return {
        ...
        "status": r["status"],
        "text": r["text"],
        "citations": _uj(r["citations_json"], []),
        "search_queries": _uj(r["search_queries_json"], []),
        "ground_truth": _uj(r["ground_truth_json"]),
...
def get_responses(
    ...
    try:
        q = (
            "SELECT resp.*, i.cluster_id AS cluster_id, i.text AS intent_text "
    ...
    try:
        conn.execute(
            ...
            (
                ...
                f["engine"],
                f.get("judge_model"),
                f.get("prompt_version"),
                f.get("top_pick"),
                _j(f.get("products", [])),
                1 if f.get("is_ground_truth") else 0,
...
def get_funnel(run_id: str) -> list[dict]:
    ...
    try:
        q = (
            "SELECT f.*, i.cluster_id AS cluster_id, i.text AS intent_text, i.attributes_json AS intent_attrs "
...
def _decision_from_row(r: dict) -> dict:
    return {
        ...
        "batch_id": r["batch_id"],
        "intent": _uj(r["intent_json"], {}),
        "candidates": _uj(r["candidates_json"], []),
        "winner": r["winner"],
        "per_product": _uj(r["per_product_json"], []),
...
def get_decisions_by_batch(batch_id: str) -> list[dict]:
    ...
    try:
        cur = conn.execute(
            "SELECT * FROM decisions WHERE batch_id=? ORDER BY created_at", (batch_id,)
        )
...
def get_batch(batch_id: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "runs": r["runs"],
            "status": r["status"],
            "n_intents": r["n_intents"],
            "shares": _uj(r["shares_json"], {}),
            "decision_ids": _uj(r["decision_ids_json"], []),
            "error": r["error"],
            "created_at": r["created_at"],
            "updated_at": r["updated_at"],
        }
...
def list_batches(product_ref: Optional[str] = None) -> list[dict]:
    ...
    try:
        if product_ref:
            cur = conn.execute(
                "SELECT batch_id FROM batches WHERE candidates_json LIKE ? ORDER BY created_at DESC",
                (f'%"{product_ref}"%',),
            )
        else:
...
def save_diagnosis(key: str, product_ref: str, source: str, payload: dict) -> None:
    ...
    try:
        conn.execute(
            "INSERT OR REPLACE INTO diagnoses (key, product_ref, source, json, generated_at) VALUES (?,?,?,?,?)",
...
def create_debate_session(
    session_id: str, product_ref: str, focus_defect_id: Optional[str], meta: dict
) -> None:
    conn = connect()
    try:
        conn.execute(
...
def get_debate_session(session_id: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "focus_defect_id": r["focus_defect_id"],
            "messages": _uj(r["messages_json"], []),
            "meta": _uj(r["meta_json"], {}),
            "created_at": r["created_at"],
        }
    finally:
        conn.close()
...
def save_debate_messages(
    session_id: str, messages: list[dict], meta: Optional[dict] = None
) -> None:
    conn = connect()
    try:
        if meta is not None:
            conn.execute(
                "UPDATE debate_sessions SET messages_json=?, meta_json=?, updated_at=? WHERE session_id=?",
                (_j(messages), _j(meta), now_iso(), session_id),
            )
        else:
            conn.execute(
                "UPDATE debate_sessions SET messages_json=?, updated_at=? WHERE session_id=?",
...
def kv_get(cache_key: str) -> Any:
    ...
    try:
        row = conn.execute(
            "SELECT value_json FROM kv_cache WHERE cache_key=?", (cache_key,)
        ).fetchone()
...
def kv_list(prefix: str) -> dict[str, Any]:
    ...
    try:
        cur = conn.execute(
            "SELECT cache_key, value_json FROM kv_cache WHERE cache_key LIKE ?", (prefix + "%",)
        )
...
def kv_set(cache_key: str, kind: str, value: Any) -> None:
    ...
    try:
        conn.execute(
            "INSERT OR REPLACE INTO kv_cache (cache_key, kind, value_json, created_at) VALUES (?,?,?,?)",
...
def save_source(url: str, domain: str, title: str, status: str, text: str) -> None:
    ...
    try:
        conn.execute(
            "INSERT OR REPLACE INTO sources (url, domain, title, status, text, fetched_at) VALUES (?,?,?,?,?,?)",
