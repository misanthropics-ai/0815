...
def main() -> int:
    ...
    for label, ref in (("a", config["baseline_ref"]), ("b", config["treatment_ref"])):
        batches[label] = api_json(args.base_url, "POST", "/simulate/batch", {
    ...
    query = urllib.parse.urlencode({"a": config["baseline_ref"], "b": config["treatment_ref"], "cluster": config["cluster_id"]})
    ...
    output = {
        ...
        "cached_request": args.cached, "config": config, "batches": batches, "compare": compare,
        "acceptance": {"minimum_delta": config["minimum_delta_recommendation"],
